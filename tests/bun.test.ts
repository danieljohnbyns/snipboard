// Test the library functions as expected when installed via bun.
...
// 2. Initialize a new bun project and install the library.
...
const createAndRunScript = async (
	script: string
): Promise<{
	dir: string;
	run: () => Promise<string>;
	...
	await runCommand('bun', ['init', '-y'], { cwd: tempDir });

	// Install the library (assuming it's published to bun)
	await runCommand('bun', ['install', '@danieljohnbyns/snipboard'], {
		cwd: tempDir
	});
...
test('bun install should work', async () => {
...
test('upload function should return a string URL', async () => {
...
(async () => {
	...
	const output = await run();
	expect(output.trim()).toMatch(/^https?:\/\/(?:i\.)?snipboard\.io\/\w+(?:\.\w+)?$/);
	cleanup();
});
